...
// Trim sticky packets (concatenated frames) in master RX buffer: keep only the first full frame
...
void ModbusInit(modbusHandler_t * modH)
{
  ...
  if (numberHandlers < MAX_M_HANDLERS)
  {
	  ...
	  else if (modH->uModbusType == MB_MASTER)
	  {
		  ...
		  {
			  printf("Error: Failed to create Modbus timeout timer, check heap and stack size\n");
			  return;
		  }
		  ...
		  {
			  printf("Error: Failed to create Modbus telegram queue, check heap and stack size\n");
			  return;
		  }
	  ...
	  {
		  printf("Error: Modbus type not supported, choose a valid Type\n");
		  return;
	  }

	  if  (modH->myTaskModbusAHandle == NULL)
	  {
		  printf("Error: Failed to create Modbus task, check heap and stack size\n");
		  return;
	  }
	  ...
	  {
		  printf("Error: Failed to create T35 timer, check heap and stack size\n");
		  return;
	  }
	  ...
	  {
		  printf("Error: Failed to create Modbus semaphore, check heap and stack size\n");
		  return;
	  }
  ...
  {
	  printf("Error: No more Modbus handlers supported, increase MAX_M_HANDLERS\n");
	  return;
  }
...
void ModbusStart(modbusHandler_t * modH)
{
	...
	{
		printf("Error: Invalid hardware type selected\n");
		return;
	}

	if (modH->xTypeHW == USART_HW_DMA && ENABLE_USART_DMA == 0  )
	{
		printf("Error: To use USART_HW_DMA you need to enable it in the ModbusConfig.h file\n");
		return;
	}
	...
	if (modH->xTypeHW == USART_HW || modH->xTypeHW ==  USART_HW_DMA )
	{
          ...
          {
          	printf("Error: Define the DATA pointer shared through Modbus for slave mode\n");
          	return;
          }
...
printf("Waiting for port to be ready\n");
          ...
          if(HAL_UART_Receive_IT(modH->port, &modH->dataRX, 1) != HAL_OK)
          {
                while(1)
                {
                    printf("error in your initialization code");
					HAL_Delay(1000);
                 }
          }
          ...
          if(modH->u8id !=0 && modH->uModbusType == MB_MASTER )
          {
        	  while(1)
        	  {
        	    printf("error Master ID must be zero");
				HAL_Delay(1000);
        	  }
  ...
  for(;;)
  {
   ...
   if(modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA)
   {
	  ...
	  if (getRxBuffer(modH) == ERR_BUFF_OVERFLOW)
	  {
	      modH->i8lastError = ERR_BUFF_OVERFLOW;
	   	  modH->u16errCnt++;
		  printf("Error: Buffer overflow, frame too large\n");
		  continue;
	  }
   ...
   // check slave id
	...
	if (u8exception > 0)
	{
		...
		{
			// Print failure banner and dump the offending request frame for debugging
			printf("P:VALIDATE,err=0x%02X,size=%u\r\n", (unsigned)u8exception, (unsigned)modH->u8BufferSize);
			if (modH->u8BufferSize >= 2) {
				printf("P:VALIDATE,id=0x%02X,func=0x%02X\r\n", modH->u8Buffer[0], modH->u8Buffer[1]);
			}
		    buildException( u8exception, modH);
			sendTxBuffer(modH);
			printf("[ModbusLib]Error: %d\n", u8exception);
		}
  ...
  for(;;)
  {
	...
	// check buffer size，if too small, return error
	  if ( modH->u8BufferSize < 6){
		  ...
		  printf("P:BAD_SIZE,size=%u", (unsigned)modH->u8BufferSize);
		  if (modH->u8BufferSize >= 2) {
			  printf("id=0x%02X func=0x%02X\r\n", modH->u8Buffer[0], modH->u8Buffer[1]);
		  }
		  xTaskNotify((TaskHandle_t)telegram.u32CurrentTask, modH->i8lastError, eSetValueWithOverwrite);
		  continue;
	  }
	  ...
	  // Try to trim sticky packets: if two (or more) frames were concatenated by a transparent link,
	  // keep only the first complete frame and discard the tail, before CRC validation.
	  mb_trim_sticky_packet(modH);
...
// Helper: compute expected RTU frame length for first response frame in buffer
// Returns 0 if cannot determine a valid length.
static uint16_t mb_expected_frame_len(const uint8_t *buf, uint16_t size)
{
	if (size < 5) return 0; // id + func + minimal payload + CRC not possible
	uint8_t id = buf[0];
	uint8_t fc = buf[1];
	(void)id; // id is not used to compute length but must be present

	// Exception response: func | 0x80, then 1-byte exception code, CRC(2)
	if (fc & 0x80) {
		return 5; // id(1) + func(1) + ex(1) + CRC(2)
	}
	...
	case MB_FC_READ_INPUT_REGISTER:
	case MB_FC_READ_REGISTERS: {
		// Response: id(1) func(1) byte_count(1) data(N) crc(2)
		if (size < 3) return 0;
		uint8_t byte_count = buf[2];
		// minimal: 1+1+1+0+2 = 5
		return (uint16_t)(1 + 1 + 1 + byte_count + 2);
	}
	case MB_FC_WRITE_COIL:
	case MB_FC_WRITE_REGISTER:
	case MB_FC_WRITE_MULTIPLE_COILS:
	case MB_FC_WRITE_MULTIPLE_REGISTERS:
		// Most write responses are echo of address/qty: fixed 8 bytes: id(1) func(1) addr(2) qty(2) crc(2)
		return 8;
	default:
		return 0;
	}
...
static void mb_trim_sticky_packet(modbusHandler_t *modH)
{
	// Only for RTU (non-TCP)
#if ENABLE_TCP == 1
	if (modH->xTypeHW == TCP_HW) return;
#endif
	uint16_t size = modH->u8BufferSize;
	uint8_t *buf = modH->u8Buffer;
	if (size < 5) return;

	// 扫描缓冲区，寻找第一个“功能码可识别且CRC正确”的完整帧，
	// 若前面有噪声（前导垃圾字节），则丢弃噪声并对尾部粘连做裁剪。
	for (uint16_t off = 0; off + 5 <= size; ++off) {
		uint16_t remain = size - off;
		uint16_t exp = mb_expected_frame_len(buf + off, remain);
		if (exp == 0 || exp > remain) {
			// 长度无法判定或剩余不足以构成完整帧，继续扫描
			continue;
		}
		// 校验该候选帧的CRC
		uint16_t rx_crc = ((uint16_t)buf[off + exp - 2] << 8) | buf[off + exp - 1];
		uint16_t calc = calcCRC(buf + off, (uint8_t)(exp - 2));
		if (rx_crc != calc) {
			// CRC 不通过，继续扫描下一个偏移
			continue;
		}
		...
		// 找到首个有效帧
		if (off > 0) {
			printf("P:RESYNC,drop_head=%u,first_frame_len=%u\r\n", (unsigned)off, (unsigned)exp);
			memmove(buf, buf + off, exp);
		}
		// 裁剪尾部粘连
		if ((off + exp) < size) {
			printf("P:STICKY,extra=%u,trimmed_len=%u\r\n", (unsigned)(size - (off + exp)), (unsigned)exp);
		}
		modH->u8BufferSize = exp;
		return;
	}
	// 未找到有效帧：保持原样交由 validateAnswer 处理（可能会返回 BAD_CRC/EXC_FUNC_CODE）
}
